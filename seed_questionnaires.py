...
from app.models.enums import (
    Gender, RelationshipGoal, EducationLevel, ProfessionalSituation, 
    ReligiousPractice, Smoker, DrinksAlcohol,
    SportFrequency, SpecificDietaryHabits, PersonalityType,
    PrimaryLoveLanguage, ImportanceOfSexuality, PartnerMustWantChildren
)
...
def get_realistic_questionnaire_data(gender: str, user_name: str):
    ...
    if is_career_focused:
        education_level = random.choice(["bac_plus_4_5", "bac_plus_2_3", "bac_plus_4_5"])  # Higher education bias
        professional_situation = random.choice(["salarie", "independant", "salarie"])  # Less likely student
    else:
    ...
    if is_religious:
        ...
        religious_practice = random.choice(["oui", "occasionnellement", "oui"])  # More likely to practice
        partner_must_share_religion = random.choice(["oui_absolument", "serait_un_plus", "serait_un_plus"])
        accept_non_believer = random.choice(["non", "oui", "non"])  # Less likely to accept
    else:
        religion = random.choice(["athee", "agnostique", "christianisme"])  # Some cultural Christians
        religious_practice = random.choice(["non", "occasionnellement", "non"])
        partner_must_share_religion = random.choice(["pas_necessaire", "serait_un_plus", "pas_necessaire"])
    ...
    data = {
        ...
        "relationship_goal": "mariage" if int(age) > 28 or random.random() > 0.6 else "relation_serieuse",
        ...
        "faith_transmission_to_children": "oui" if is_religious else random.choice(["non", "je_ne_sais_pas_encore"]),
        ...
        "partner_share_convictions_importance": random.choice(["tres_important", "important", "peu_important"]),
        
        # Lifestyle (health-conscious logic)
        "sport_frequency": (
            random.choice(["3_5_par_semaine", "tous_les_jours", "1_2_par_semaine"]) if is_health_conscious
            else random.choice(["jamais", "1_2_par_semaine", "3_5_par_semaine"])
        ),
        "specific_dietary_habits": (
            random.choice(["vegetarien", "vegan", "aucune"]) if is_health_conscious and random.random() > 0.7
            else ("halal" if religion == "islam" else "aucune")
        ),
        "hygiene_tidiness_approach": random.choice(["tres_important", "important", "flexible"]),
        "smoker": "non" if is_health_conscious else random.choice(["oui", "non", "occasionnellement"]),
        "drinks_alcohol": (
            "non" if religion == "islam" or (is_health_conscious and random.random() > 0.6)
        ...
        "partner_sport_frequency": (
            random.choice(["1_2_par_semaine", "3_5_par_semaine", "peu_importe"]) if is_health_conscious
            else "peu_importe"
        ),
        "partner_same_dietary_habits": "oui" if religion == "islam" else "peu_importe",
        "partner_cleanliness_importance": random.choice(["tres_important", "important", "flexible"]),
        "accept_smoker_partner": "non" if is_health_conscious else random.choice(["oui", "non", "occasionnellement_seulement"]),
        "accept_alcohol_consumer_partner": (
            "non" if religion == "islam" else
            ("occasionnellement_seulement" if is_health_conscious else "oui")
        ),
        ...
        "primary_love_language": random.choice(["mots_affirmation", "temps_qualite", "contact_physique", "cadeaux", "services"]),
        "friends_visit_frequency": (
            "souvent" if personality_type == "extraverti"
            else ("rarement" if personality_type == "introverti" else "parfois")
        ),
        "tolerance_social_vs_homebody": (
            "tres_social" if personality_type == "extraverti"
        ...
        "ideal_intimate_frequency": random.choice(["quotidienne", "plusieurs_fois_semaine", "hebdomadaire"]),
        "comfort_level_talking_sexuality": random.choice(["tres_confortable", "confortable", "inconfortable"]),
        "comfortable_public_affection": random.choice(["oui", "non", "modere"]),
        
        # Compatibility
        "partner_similarity_preference": random.choice(["tres_similaire", "quelques_differences", "complementaire"]),
        "partner_age_range": f"{max(18, int(age) - 5)}-{min(60, int(age) + 8)}",
        
        # Socio-economic
        "importance_financial_situation_partner": random.choice(["tres_important", "important", "peu_important"]),
        "ideal_partner_education_profession": random.choice(["similaire", "superieur", "peu_importe"]),
...
def create_questionnaire_for_user(user_id: int, gender: str, user_name: str):
    ...
    try:
        ...
        existing = session.query(Questionnaire).filter(
        ...
        print(f"     Location: {data['city_of_residence']}, Profession: {data['professional_situation']}")
