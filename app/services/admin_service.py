...
class AdminService(BaseService):
    ...
    def get_journeys(self, status: str = None, current_step: int = None, skip: int = 0, limit: int = 100) -> List[Journey]:
        """
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        new_users_last_week = self.db.query(func.count(User.id)).filter(
            User.created_at >= datetime.utcnow() - timedelta(days=7)
        ).scalar()
        
        # Match stats
        total_matches = self.db.query(func.count(Match.id)).scalar()
        pending_matches = self.db.query(func.count(Match.id)).filter(Match.status == "pending").scalar()
        confirmed_matches = self.db.query(func.count(Match.id)).filter(Match.status == "matched").scalar()
        declined_matches = self.db.query(func.count(Match.id)).filter(Match.status == "declined").scalar()
        
        # Journey stats
        total_journeys = self.db.query(func.count(Journey.id)).scalar()
        active_journeys = self.db.query(func.count(Journey.id)).filter(Journey.status == "active").scalar()
        completed_journeys = self.db.query(func.count(Journey.id)).filter(Journey.status == "completed").scalar()
        ended_journeys = self.db.query(func.count(Journey.id)).filter(Journey.status == "ended").scalar()
        
        # Message stats
        total_messages = self.db.query(func.count(Message.id)).scalar()
        messages_last_week = self.db.query(func.count(Message.id)).filter(
            Message.created_at >= datetime.utcnow() - timedelta(days=7)
        ).scalar()
        
        # Meeting stats
        total_meetings = self.db.query(func.count(MeetingRequest.id)).scalar()
        accepted_meetings = self.db.query(func.count(MeetingRequest.id)).filter(
