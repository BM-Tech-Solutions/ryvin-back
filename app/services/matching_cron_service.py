...
class MatchingCronService:
    ...
    async def run_daily_matching(self) -> dict:
        ...
        try:
            ...
            logger.info(
                f"Daily matching completed. Created {stats['new_matches_created']} new matches"
            )
    ...
    def _is_user_eligible_for_matching(self, user: User) -> bool:
        ...
        questionnaire = (
            self.session.query(Questionnaire).filter(Questionnaire.user_id == user.id).first()
        )
    ...
    async def _process_user_matching(self, user: User) -> int:
        ...
        for potential_match_user, potential_match_questionnaire in potential_matches:
            try:
                ...
                if (
                    compatibility_result.total_score >= self.min_compatibility_score
                    and not compatibility_result.deal_breaker_failed
                ):
                    # Create the match
                    match = self._create_match(
                        user.id, potential_match_user.id, compatibility_result
                    )

                    if match:
                        matches_created += 1
                        logger.debug(
                            f"Created match between {user.id} and {potential_match_user.id} "
                            f"with score {compatibility_result.total_score}"
                        )

            except Exception as e:
                logger.error(
                    f"Error calculating compatibility between {user.id} and "
    ...
    def _get_potential_matches_for_user(
        ...
        opposite_gender = (
            Gender.FEMME if user_questionnaire.gender == Gender.HOMME else Gender.HOMME
        )
        ...
        existing_match_user_ids = (
            ...
            .union_all(self.session.query(Match.user1_id).filter(Match.user2_id == user.id))
            .all()
        )
    ...
    def _create_match(
        self, user1_id: UUID, user2_id: UUID, compatibility_result
    ) -> Optional[Match]:
        """
    ...
    def get_matching_statistics(self) -> dict:
        ...
        users_with_questionnaire = (
            self.session.query(User)
            .filter(User.is_active.is_(True), User.has_completed_questionnaire.is_(True))
            .count()
        )
        ...
        active_matches = (
            self.session.query(Match).filter(Match.status.in_(["pending", "active"])).count()
        )
        ...
        return {
            ...
            "matching_rate": (users_with_questionnaire / total_users * 100)
            if total_users > 0
            else 0,
        }
