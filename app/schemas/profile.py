...
class ProfileBase(BaseModel):
    ...
    def validate_genre(cls, v):
        if v not in [gender.value for gender in Gender]:
            raise ValueError(f"Invalid gender. Must be one of: {[gender.value for gender in Gender]}")
        return v
    
    @validator("situation_professionnelle")
    def validate_situation_professionnelle(cls, v):
        if v not in [status.value for status in ProfessionalStatus]:
            raise ValueError(f"Invalid professional status. Must be one of: {[status.value for status in ProfessionalStatus]}")
        return v
    
    @validator("niveau_etudes")
    def validate_niveau_etudes(cls, v):
        if v not in [level.value for level in EducationLevel]:
            raise ValueError(f"Invalid education level. Must be one of: {[level.value for level in EducationLevel]}")
        return v
    
    @validator("type_relation_recherchee")
    def validate_type_relation_recherchee(cls, v):
        if v not in [rel_type.value for rel_type in RelationshipType]:
            raise ValueError(f"Invalid relationship type. Must be one of: {[rel_type.value for rel_type in RelationshipType]}")
        return v
    
    @validator("genre_recherche")
    def validate_genre_recherche(cls, v):
        if v not in [gender.value for gender in Gender]:
            raise ValueError(f"Invalid gender preference. Must be one of: {[gender.value for gender in Gender]}")
        return v
    
    @validator("nombre_enfants")
